(function registerNarrator(DSL) {
  ...
  // Saved preferences and resume points (DSL.store adds the "dsl-" prefix).
  function readJson(key, fallback) {
    const saved = DSL.store.get(key, {});
    return { ...fallback, ...(saved && typeof saved === "object" ? saved : {}) };
  }

  const writeJson = (key, value) => DSL.store.set(key, value);
  ...
  function run({ lessonId, title, chapters, onLab }) {
    ...
    const saveKey = `narrated-${lessonId}`;
  ...
  // Chapter helpers shared by every narrated lesson.

  // Put a storyboard ({ build, frames }) on stage; frame(line, i) speaks a line while frame i animates.
  function story(n, scene, board) {
    scene.innerHTML = `<div class="gd-story-stage"></div>`;
    const ctx = board.build(scene.firstElementChild);
    return (line, i) => Promise.all([n.say(line), n.show(() => board.frames[i].enter(ctx, n))]);
  }
  ...
  // The end-of-lesson quiz: the interviewer asks, the tutor explains wrong answers.
  // Needs lines quiz.intro, quiz.q1…qN (with -why), quiz.right1-3, quiz.pass, and quiz.retry.
  function quizChapter({ questions, passScore = Math.ceil(questions.length * 0.75), onPass }) {
    ...
    return {
      ...
      async script(n, scene) {
        scene.innerHTML = `<div class="gd-quiz nr-quiz">
          ...
          <div class="nr-marks">${questions.map(() => "<i></i>").join("")}</div>
        </div>`;
        ...
        count.textContent = `${right} / ${questions.length}`;
  ...
  DSL.Narrator = Object.freeze({ run, register, keywords, story, throttled, quizChapter });
