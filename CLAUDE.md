...
- Phase 3 (Level 1): Writing SQL module complete (lessons 02–07). Module **Keys and data models**: `keys` (08), `models` (09; dataset `bakeryModels` adds `order_docs` JSON documents and a `referrals` graph). Remaining: `p3-latency` (new module Measuring systems). Exercises that build schema use the lab's `probe` option (a check query run after the learner's SQL) and datasets `bakeryStrict` (foreign keys on), `bakeryOrphan`, `strict`. The last lesson before Storage owns the "Next up: how a database reads rows from disk" line.
