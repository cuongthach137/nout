...
- Phase 3 (Level 1): the **Writing SQL module is complete** (`select`, `joins`, `group`, `sub`, `window`, `null`, lessons 02–07), after Data modeling. Shared SQL scene helpers: `js/components/sql-scenes.js`; engine `js/components/sql.js`. Remaining Phase 3 items: `p3-rel` (relationships and keys), `p3-models` (relational/document/graph), `p3-latency`. `null`'s `finish.2` still says "Next up: how a database reads rows from disk" (Storage); update it if a lesson is inserted after it.
